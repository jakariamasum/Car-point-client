...
const ListingDetails = () => {
  ...
  useEffect(() => {
    ...
    axiosSecure
      .get(`/getSingleSavedAd/${id}?email=${dbCurrentUser?.email}`)
      .then((res) => {
        const data = res.data;
        if (data) {
          setPostSaved(true);
  ...
  return (
    <div className="container mx-auto p-5 flex flex-col justify-center items-center gap-10 font-body overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-center items-start gap-14 w-full lg:w-5/6 mt-[80px]">
        ...
        >
          ...
          <div className="flex justify-center items-center gap-3 mt-3">
            <p className="bg-gray text-black px-3 py-1 rounded-[4px] w-fit">
              {carBrand}
            </p>
            <p className="bg-gray text-black px-3 py-1 rounded-[4px] w-fit">
              {carType}
          ...
          <div className="mt-4 flex justify-center items-center gap-4">
            ...
            {!postSaved ? (
              <button
                ...
                className="flex justify-center items-center gap-2 text-lightBlack font-medium border-[1px] border-gray rounded px-3 py-1 hover:border-main hover:text-main duration-300"
              >
                <FaRegStar />
                Save ad
              </button>
              ...
              <button
                onClick={() => handleRemoveSaved(_id)}
                className="flex justify-center items-center gap-2 text-main font-medium border-[1px] border-main rounded px-3 py-1 hover:border-lightBlack hover:text-lightBlack duration-300"
              >
                <FaStar />
                Saved
              </button>
            ...
            {sellStatus === "sold" || sellerEmail === dbCurrentUser?.email ? (
              <button
                disabled
                className="flex justify-center items-center gap-2 text-lightBlack font-medium border-[1px] border-gray rounded px-3 py-1 disabled:opacity-40 cursor-not-allowed"
              >
                <AiTwotoneLike /> Place a bid
              </button>
            ) : (
              <button
                onClick={() =>
                ...
                className="flex justify-center items-center gap-2 text-lightBlack font-medium border-[1px] border-gray rounded px-3 py-1 hover:border-sub hover:text-sub duration-300"
              >
                <AiTwotoneLike /> Place a bid
              </button>
            )}
            ...
            <span>
              {showNumber === false ? (
                <button
                  ...
                  className="bg-[#17bbec] text-white px-2 py-1 rounded"
                >
                  Show number
                </button>
              ) : (
          ...
          <p
            className={`${
              sellerVerificationStatus !== "verified"
                ? "text-[red] bg-[#ffd6d6]"
                : "text-[green] bg-[#daffda]"
            } px-4 py-1 rounded-md font-medium flex justify-start items-center gap-2 capitalize`}
          >
      ...
      <dialog
        id="biddingModal"
        className="modal modal-bottom sm:modal-middle font-heading"
      >
        <div className="modal-box w-full flex flex-col justify-center items-center">
          ...
          <div className="modal-action w-full">
            <form
              ...
              method="dialog"
              className="w-full flex flex-col justify-start items-start p-5 gap-2"
            >
              {/* name */}
