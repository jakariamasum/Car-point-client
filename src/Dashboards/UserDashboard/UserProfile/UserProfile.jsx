...
const UserProfile = () => {
  ...
  const { dbCurrentUserPending, dbCurrentUser, dbCurrentUserRefetch } =
    useCurrentUser();
  ...
  const handleVerificationRequest = () => {
    ...
    axiosSecure
      .put(`/updateUserDetails/${dbCurrentUser._id}`, verificationRequest)
      .then((res) => {
        const data = res.data;
  ...
  const handleUpdateDetails = (e) => {
    ...
    axiosSecure
      .put(`/updateUserDetails/${dbCurrentUser._id}`, userDetails)
      .then((res) => {
        const data = res.data;
        if (data.modifiedCount > 0) {
  ...
  const handleFeedbackSubmit = (e) => {
    ...
    const feedbackDetails = {
      ...
      feedBack,
      feedbackBy,
      feedbackProvidingDate,
      feedbackProvider,
      feedbackProviderEmail,
      feedbackProviderPhoto,
    };

    // send the feedback to server and db
    axiosSecure
      .post("/userFeedback", feedbackDetails)
      .then((res) => {
        const data = res.data;
        if (data.insertedId) {
    ...
    <div className="lg:min-h-[100vh] w-full flex flex-col justify-start items-center gap-5">
      ...
      <div className="flex flex-col justify-center items-center gap-3">
        ...
        dbCurrentUser?.verifyStatus === "declined" ? (
          <div className="flex flex-col justify-center items-center gap-3">
            <p className="flex justify-center items-center gap-1 text-[18px] text-[red] font-medium capitalize">
              <CgDanger /> {dbCurrentUser?.verifyStatus}
            </p>
            {dbCurrentUser?.verificationRequest !== "requested" ? (
              <button
                onClick={handleVerificationRequest}
                className="bg-main px-5 py-2 rounded-[5px] text-white font-medium hover:bg-sub duration-300"
              >
          ...
          <p className="flex justify-center items-center gap-1 text-[18px] text-[#0da50d] font-medium">
            <CgCheckO />
            Verified
          </p>
        )}
            ...
            <div className="w-full flex flex-col justify-start items-start gap-5">
              ...
              >
                ...
                <div className="w-full flex flex-col justify-start items-start gap-8">
                  ...
                  <div className="font-body flex flex-col justify-start items-start gap-3 w-full">
                    ...
                    <textarea
                      ...
                      id="address"
                      name="address"
                      placeholder="Enter your proper address here"
                      className="w-full border-[1px] border-gray px-4 py-2 rounded-[3px] focus:outline-none focus:border-lightMain"
                    ></textarea>
                  </div>
                </div>
                ...
                <input
                  ...
                  className="w-fit px-4 py-2 bg-black mt-5 rounded-md text-white hover:bg-sub duration-300 font-semibold cursor-pointer"
                />
              </form>
            </div>
          ) : (
        ...
        <div className="w-full md:w-1/2 self-stretch">
          {singleFeedback ? (
            <div className="w-full h-full flex flex-col justify-center items-start gap-4 bg-[#ececec] p-5 shadow-[0_0_50px_#ececec] rounded-[5px]">
              ...
              <p className="font-medium text-lightBlack flex justify-center items-center gap-1">
                <span className="text-black">Rating:</span>{" "}
                {singleFeedback?.ratingByUser} <FaStar />{" "}
              </p>
              ...
              <p className="font-medium text-lightBlack">
                <span className="text-black">Provided on:</span>{" "}
                {singleFeedback?.feedbackProvidingDate}
              </p>
            </div>
          ) : (
            <div className="w-full flex flex-col justify-start items-start gap-5">
              ...
              >
                ...
                <div className="w-full flex flex-col justify-start items-start gap-8">
                  ...
                  <div className="font-body flex flex-col justify-start items-start gap-3 w-full">
                    <label className="font-medium">
                      Provide a rating <span className="text-[red]">*</span>
                    </label>
                    <select
                      required
                      id="ratingByUser"
                  ...
                  <div className="font-body flex flex-col justify-start items-start gap-3 w-full">
                    ...
                    <textarea
                      ...
                      id="feedbackByUser"
                      name="feedbackByUser"
                      placeholder="Your valuable feedback about out platform"
                      className="w-full border-[1px] border-gray px-4 py-2 rounded-[3px] focus:outline-none focus:border-lightMain"
                    ></textarea>
                  </div>
                </div>
